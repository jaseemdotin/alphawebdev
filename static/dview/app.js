...
const BigImage = document.getElementById('imgnew');
const imgSlider = document.getElementById('img-slider');

imgSlider.addEventListener('click', event => {

    if (event.target === Nomos1) {
         BigImage.setAttribute ("src","{{ins.pimage1.url}}");
    }

    else if (event.target === Nomos2) {
        BigImage.setAttribute ("src","{{ins.pimage2.url}}");
    }

    else {
    BigImage.setAttribute ("src","{{ins.pimage3.url}}");
    }
